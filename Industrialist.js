...
if (a1 != null) {
	...
	$('#search_state').on('click', function () {
		...
		$.post("industry_search.php", data, function (stat) {
			...
			for (var i = 0; i < a.length; i++) {
				b = a[i];
				var c = "r" + i; var k = "b" + i;
				$("#tb").after("<tr id =" + c + "><td>" + (a.length - i) + "</td><td>" + b["f_name"] + "</td><td>" + b["quantity_kg"] + "</td><td>" + b["price_per_kg"] + "</td><td>" + b["f_mobile_no"] + "</td><td>" + b["location"] + "</td><td><img src=\"farmercoirpics/" + b["url"] + "\"></td><td><button type=\"button\" id =" + k + " class=\"btn btn-outline-secondary abcd\" onclick=placeOrderClicked(" + k + ")>place order</button></td></tr>");
			}
