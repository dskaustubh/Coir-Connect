...
if(a1!=null)
{
	...
	$("#navbarDropdown").html("HI       "+a1["user_name"]+ " !!!!!");
 	...
 	data=null;
	  	...
	  	$.post("sortbyprice.php", data, function(stat) {
        ...
        $("#"+r).remove();
			...
			{ 
				 b=a[i];
				var c="r"+i;var k="b"+i;
				$("#tb").after("<tr id ="+c+"><td>"+(a.length-i)+"</td><td>"+b["name"]+"</td><td>"+b["quantity_kg"]+"</td><td>"+b["price_per_kg"]+"</td><td>"+b["mobile_no"]+"</td><td>"+b["location"]+"</td><td><img src=\"farmercoirpics/"+b["url"]+"\"style=\"width:30vh;height:10vw;\"></td><td><button type=\"button\" id ="+k+" class=\"btn btn-outline-secondary abcd\" onclick=placeOrderClicked(" + k +")>place order</button></td></tr>") ;   
			}
